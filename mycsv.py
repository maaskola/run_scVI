...
class CsvDataset(GeneExpressionDataset):
    ...
    def __init__(self, filename, save_path='data/', url=None, new_n_genes=600, subset_genes=None,
        ...
        # self.mylabels = np.array(list(data.index), dtype=str)
        # self.mylabels = data.index.str
        # TODO
        # labels = GeneExpressionDataset.arrange_categories(data.index.str)
        # data = data.values
        print(data)
    ...
    def preprocess(self):
        ...
        self.spot_names = np.array(list(data.index), dtype=str)
        print(data)
        print(data.index)
    ...
    def __init__(self, save_path='data/', index=11, new_n_genes=600):
        super(MouseOBDataset, self).__init__(f"Rep{index}_MOB_count_matrix-1.tsv", save_path=save_path,
                                             ...
                                             sep='\t', gene_by_cell=False, new_n_genes=new_n_genes)
                                                 # TODO we can specify the attrs x_coord and y_coord
